...
build/openmaptiles.tm2source/data.yml:
	mkdir -p build/openmaptiles.tm2source && generate-tm2source hiking.yaml --host="postgres" --port=5432 --database="openmaptiles" --user="openmaptiles" --password="openmaptiles" > build/openmaptiles.tm2source/data.yml

build/mapping.yaml:
	mkdir -p build && generate-imposm3 hiking.yaml > build/mapping.yaml

build/tileset.sql:
	mkdir -p build && generate-sql hiking.yaml > build/tileset.sql
